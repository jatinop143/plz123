...
const FinishScreen = ({ params }) => {
  ...
  return (
    <div className="px-10 md:px-20 lg:px-44 my-7">
      ...
      <h2 className='md:text-center text-sm md:text-lg text-gray-400 border p-2 rounded-md md:flex md:items-center md:gap-5 mt-2'>{process.env.NEXT_PUBLIC_HOST_URL}/course/{sessionData?.CourseId}/start
      <HiOutlineClipboardDocumentCheck className='h-5 w-5 cursor-pointer' onClick={async ()=> await navigator.clipboard.writeText(process.env.NEXT_PUBLIC_HOST_URL+'/course/'+sessionData?.CourseId+'/start')} /></h2>
    </div>
  );
};
