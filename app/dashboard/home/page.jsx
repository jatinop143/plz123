...
import { HappyIndexResults, CourseList } from '../../../configs/Schema';
...
function HappyIndexAssessment() {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        // Try to get username from CourseList first
        const courseData = await db.select({
          username: CourseList.Username
        })
          .from(CourseList)
          .where(eq(CourseList.createdBy, userId))
          .limit(1);

        if (courseData.length > 0 && courseData[0].username) {
          setUsername(courseData[0].username);
        } else {
          // Fallback to user's full name or email
          setUsername(userFullName || userEmail || 'User');
        }
      } catch (error) {
    ...
    const fetchResults = async () => {
      ...
      setPreviousResults(combinedResults);
      setIsLoading(false);
    };

    fetchUserData();
    fetchResults();
  }, [isLoaded, userId, userEmail, userFullName]);
  ...
  const calculateHappyIndex = async () => {
    ...
    if (userId) {
      try {
        await db.insert(HappyIndexResults).values({
          userId: userId,
          username: username,
          score: index,
          answers: answers
  ...
  const clearHistory = () => {
    ...
    // Optionally clear from database if authenticated
    if (userId) {
      db.delete(HappyIndexResults)
        .where(eq(HappyIndexResults.userId, userId))
        .catch(error => console.error('Error clearing database history:', error));
    }
  };
    ...
    return (
      <div className="landing-screen">
        <div className="landing-content">
          ...
          {username && <p className="welcome-message">Hello, {username}!</p>}
  ...
  if (viewMode === 'history') {
    return (
      <div className="happy-index-container">
        ...
        {username && <p className="user-greeting">Hello, {username}</p>}
              ...
              <div className="history-list">
                {previousResults.map((result, index) => (
                  <div key={index} className="history-item">
                    ...
                    {result.username && <span className="username">{result.username}</span>}
                  </div>
                ))}
              </div>
            </>
